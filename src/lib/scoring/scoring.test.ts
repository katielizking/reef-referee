...
describe("pea puffer group care", () => {
  const puffer = () => species({id:"pea",common_name:"Pea puffer",scientific_name:"Carinotetraodon travancoricus",temperament:"aggressive",is_schooling:false,min_group_size:1});
  it("recognises group care even in an old saved species snapshot", () => {
    const s=scoreTank(tank({plant_density:"heavy",species:[{species:puffer(),quantity:6}]}));
  ...
  it("does not approve a full group in an undersized tank",()=>{
    const s=scoreTank(tank({length_cm:40,width_cm:25,height_cm:30,species:[{species:puffer(),quantity:6}]}));
