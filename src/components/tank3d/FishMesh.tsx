...
function zoneY(zone: "top" | "mid" | "bottom", height: number, substrateY: number): number {
...
export function FishGroup({
  ...
  const instances = useMemo<FishInstance[]>(() => {
    const yCentre = centerY ?? zoneY(species.swim_zone, interior.y, interior.substrateY);
...
export function ProceduralFish({
  ...
  useFrame(({ clock }, delta) => {
    ...
    g.rotation.x = foragePitch + Math.sin(t * 0.42 + phase * 0.7) * behaviour.maxPitch * 0.15;
