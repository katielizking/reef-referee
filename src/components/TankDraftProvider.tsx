...
interface DraftContext {
  state: TankState;
  setState: Dispatch<SetStateAction<TankState>>;
  savedId?: string;
  setSavedId: Dispatch<SetStateAction<string | undefined>>;
  source?: string;
...
export function TankDraftProvider({ children }: { children: ReactNode }) {
  ...
  const storageStatus = storageFailed
    ? "Device storage unavailable"
    : hydrated && savedSnapshot === snapshot
      ? "Saved on this device"
      : "Saving draft…";
  return (
    <Context.Provider
      value={{ state, setState, savedId, setSavedId, source, setSource, hydrated, storageStatus }}
    >
      {children}
    </Context.Provider>
  );
