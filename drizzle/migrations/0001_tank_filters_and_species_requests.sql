-- Multiple filters per tank
CREATE TABLE public.tank_filters (
  ...
  PRIMARY KEY (tank_id, position)
);

GRANT SELECT, INSERT, UPDATE, DELETE ON public.tank_filters TO authenticated;
GRANT ALL ON public.tank_filters TO service_role;

ALTER TABLE public.tank_filters ENABLE ROW LEVEL SECURITY;

CREATE POLICY "tank_filters select own" ON public.tank_filters
  FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.tanks t WHERE t.id = tank_filters.tank_id AND t.user_id = auth.uid()));

CREATE POLICY "tank_filters insert own" ON public.tank_filters
  FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM public.tanks t WHERE t.id = tank_filters.tank_id AND t.user_id = auth.uid()));

CREATE POLICY "tank_filters update own" ON public.tank_filters
  FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM public.tanks t WHERE t.id = tank_filters.tank_id AND t.user_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM public.tanks t WHERE t.id = tank_filters.tank_id AND t.user_id = auth.uid()));

CREATE POLICY "tank_filters delete own" ON public.tank_filters
  FOR DELETE TO authenticated
  USING (EXISTS (SELECT 1 FROM public.tanks t WHERE t.id = tank_filters.tank_id AND t.user_id = auth.uid()));

-- Missing-species requests. Write-only from the app, read only by maintainers.
CREATE TABLE public.species_requests (
...
GRANT ALL ON public.species_requests TO service_role;

ALTER TABLE public.species_requests ENABLE ROW LEVEL SECURITY;

CREATE OR REPLACE FUNCTION public.submit_species_request(p_common_name text, p_note text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $function$
DECLARE
  v_name text := trim(p_common_name);
  v_note text := nullif(trim(coalesce(p_note, '')), '');
BEGIN
  IF length(v_name) < 2 OR length(v_name) > 120 THEN
    RAISE EXCEPTION 'Enter the fish name';
  END IF;
  IF v_note IS NOT NULL AND length(v_note) > 500 THEN
    RAISE EXCEPTION 'Keep the note under 500 characters';
  END IF;
  INSERT INTO public.species_requests(common_name, note) VALUES (v_name, v_note);
END;
$function$;

-- Shared tanks now carry their full filter list.
...
REVOKE ALL ON FUNCTION public.get_shared_tank(text) FROM PUBLIC, anon, authenticated;
